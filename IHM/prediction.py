...
def get_most_probably_emotion(pred: np.ndarray) -> str:
    return get_label_from_id(np.argmax(pred))
...
def get_emotions_from_face(face) -> Union[list, str]:
    if face is not None:
        pred = model.predict(x=face)
        emotions = get_sorted_results(pred)
        return emotions
...
if __name__ == "__main__":
    ...
    face_shape = (224, 224)
    for frame, emotion in camera_modified((224, 224)):
